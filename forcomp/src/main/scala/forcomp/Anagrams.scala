...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    // best we can do with groupby is to group all the characters together,
    // e.g. (c -> List(c, c, c)).
    val g = w.toList.groupBy(c => c.toLower) map (x => (x._1, x._2.length))
    g.toList.sortBy(p => p._1)
  }
...
//    val dict = List("spiroagnew", "growapenis", "pickles")
  ...
  def anagramHelper(occ: Occurrences): List[Sentence] = {
    if (occ.isEmpty) {
      ...
      for (i <- combs if dictionaryByOccurrences.keySet(i);
           ...
           s <- anagramHelper(subtract(occ, i))) yield {j :: s}
    }
  }
