...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] =
      chars groupBy(c => c) map { case (k, v) => k -> v.size } toList
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      freqs map { case (c, x) => new Leaf(c, x)} sortWith (_.weight < _.weight)
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      if (trees.isEmpty || singleton(trees)) trees
        ...
        val combo = makeCodeTree(trees.head, trees.tail.head)
        combo :: rest sortWith (weight(_) < weight(_))
      }
    }
   ...
   */
  def until(test: List[CodeTree] => Boolean, combine: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]):
  List[CodeTree] = {
    if (test(trees)) trees
    else until(test, combine)(combine(trees))
  }
   ...
   */
    ...
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def decode_helper(subtree: CodeTree, bits: List[Bit], acc: List[Char]): List[Char] = {
        subtree match {
          case Leaf(x, _) => x :: decode_helper(tree, bits, acc)
          case Fork(left, right, _, _) => {
            if (bits.isEmpty) acc
            else if (bits.head == 0) decode_helper(left, bits.tail, acc)
            else decode_helper(right, bits.tail, acc)
          }
        }
      }
   ...
   */
  // TODO order the code table to permit binary search.
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encode_with_table(table: CodeTable)(text: List[Char], acc: List[Bit]): List[Bit] = {
        if (text.isEmpty) acc
        else codeBits(table)(text.head) ++ encode_with_table(table)(text.tail, acc)
      }
      encode_with_table(convert(tree))(text, List[Bit]())
    }
  }
